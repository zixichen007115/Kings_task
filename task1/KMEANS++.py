...
class KMEANS:
    ...
    def cluster(self, x):
        ...
        self.initial_centers(x)

        while True:
            self.nearest_center(x)
            self.calculate_center(x)
            print(self.per_change)
            if self.per_change < 1e-3 or self.max_iter < self.count:
                break
            self.count += 1
    ...
    def choose_sample(self, x):
        ...
        max_sample_list = np.zeros(self.n_cluster).astype(int)
        label = self.label
    ...
    def initial_centers(self,x):
        centers_list = np.random.randint(x.shape[0])
        centers = x[centers_list].expand(1,x.shape[1])
        left_list = np.setdiff1d(np.arange(60000),centers_list)
        left_point = x[left_list]
        for i in range(self.n_cluster-1):

            dis = torch.mm(left_point,centers.t())
            dis = dis.cpu().numpy()

            denominator = dis.sum()
            point = np.random.rand()*denominator
            be_chosen = np.searchsorted(np.cumsum(dis),point)
            be_chosen = np.min([be_chosen,left_list.shape[0]])
            centers_list = np.append(centers_list, be_chosen)

            centers = x[centers_list]
            left_list = np.setdiff1d(np.arange(60000),centers_list)
            left_point = x[left_list]
...
if __name__ == '__main__':
    ...
    if method == 'pca':
        in_dataset = 'pc.npy'
        out_dataset = 'pca++_samples.npy'
    elif method == 'ae':
        in_dataset = 'latent_rep.npy'
        out_dataset = 'ae++_samples.npy'
